...
space = np.zeros([max_size, max_size, max_size], dtype='uint8')

space[mid-size//2:mid+(size+1)//2, mid-size//2:mid+(size+1)//2, mid] = init

def update(space, new_space=None):
    if new_space is None:
        new_space = space.copy()
    
    for i, j, k in np.ndindex(*space.shape):
        space_ijk = space[i, j, k]
        ilow = max(i-1, 0)
        ...
        subspace_ijk = space[ilow:ihigh, jlow:jhigh, klow:khigh]
        count = subspace_ijk.sum() - space_ijk
        if space_ijk == 1:
            new_space[i, j, k] = 1 if (count in (2, 3)) else 0
        else:
            new_space[i, j, k] = 1 if (count == 3) else 0
